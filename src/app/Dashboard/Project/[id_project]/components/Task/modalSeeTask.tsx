...
import { getTimestampFormattedDate } from '@/Utils/getTimestampFormattedDate';
...
function ModalSeeTask({ taskSelected, modalSeeTask, setModalSeeTask }: Props) {
  ...
  const attributeTo = projectContext?.coworkers.find(
    (data) => data.id === taskSelected?.idExecuter
  );
  const createFrom = projectContext?.coworkers.find(
    (data) => data.id === taskSelected?.idOwner
  );
  return (
          ...
          <Dialog.Content className='z-20 data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-[#282828] py-[20px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none'>
            ...
            <div className='px-[20px] mt-[10px]'>
              <div className='flex flex-col gap-y-[5px] text-[18px]'>
                <p>
                  <span className='font-[500]'>Título:</span>{' '}
                  {taskSelected?.title}
                </p>
                <p>
                  <span className='font-[500]'>Descrição:</span>{' '}
                  {taskSelected?.description}
                </p>
                <p>
                  <span className='font-[500]'>Atribuido para:</span>{' '}
                  {attributeTo?.username}
                </p>
                <p>
                  <span className='font-[500]'>Status:</span>{' '}
                  {taskSelected?.status === 'concluded'
                    ? 'Concluído'
                    : 'Aberto'}
                </p>
                <p>
                  <span className='font-[500]'>Criado por:</span>{' '}
                  {createFrom?.username}
                </p>
                <p>
                  <span className='font-[500]'>Criado:</span>{' '}
                  {getTimestampFormattedDate(taskSelected?.createdDate)}
                </p>
              </div>
            </div>
