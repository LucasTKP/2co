  ...
  if (userContext.id.length > 0) {
      ...
      <>
        {userContext.repositorys
          ...
          <div className='w-full flex flex-wrap gap-[50px] mt-[25px] max-md:justify-center max-lsm:justify-start pb-[10px]'>
            {userContext.repositorys
              .filter((repository) =>
                userContext.projects.includes(repository.id)
              )
              .filter((project: any) =>
                project.name
                  .toLocaleLowerCase()
              ...
              .map((project: Repository) => {
                return (
                  <div
                    ...
                    key={project.id}
                    className='w-[300px] h-[150px] border-[1px] border-emerald-500 rounded-[5px] p-[10px] group hover:border-white hover:bg-zinc-700 cursor-pointer duration-200'
                  >
                    ...
                    {project.url && (
                      <Link href={project.url} target='__blank'>
                        <p className='truncate w-full text-neutral-400'>
                          Url: {project.url}
                        </p>
                      </Link>
                    )}
                    ...
                    <p className='text-neutral-400 mt-[5px] w-full truncate'>
          ...
          <p className='text-center font-poiretOne mt-[40px] max-lg:mt-[20px] text-[50px] max-xl:text-[45px] max-lg:text-[45px] max-md:text-[35px] max-sm:text-[30px] max-lsm:text-[25px]'>
            Ops... <br /> Parece que você ainda nao importou nenhum projeto,
            clique em adicionar e importe o primeiro!
          </p>
        )}
      </>
    );
    ...
    return (
      <div className='w-full h-full flex justify-center mt-[100px]'>
        <svg
          className='h-[50px] w-[50px] animate-spin text-white'
